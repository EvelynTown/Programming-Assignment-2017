...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs) {
	...
	if(!overflow){ //printf("%llx\n",sig_grs);
		/* TODO: round up and remove the GRS bits */
		uint32_t grs = sig_grs&(~(~0<<3));
		if((grs > 4) || (grs == 4 && (sig_grs>>3)%2)){ 
				sig_grs += 8;
...
uint32_t internal_float_add(uint32_t b, uint32_t a) {
	...
	if(fb.exponent == 1 && fa.exponent == 0) 
		shift = 0;
	else if(fb.exponent > 1 && fa.exponent == 0)
		shift--;
...
uint32_t internal_float_mul(uint32_t b, uint32_t a) {
	...
	//printf("sign_before_normalize = %x\n", f.sign);
	return internal_normalize(f.sign, exp_res, sig_res);
}
