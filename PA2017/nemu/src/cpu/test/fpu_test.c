...
void fpu_test_add() {
	...
	for(i = 0 ; i < sizeof(input)/sizeof(float) ; i++) {
		for(j = 0 ; j < sizeof(input)/sizeof(float) ; j++) {
			...
			//printf("float add ua = %x, ub = %x, res = %x, res_fpu = %x\n", a.val, b.val, res.val, res_fpu.val);		
...
void fpu_test_mul() {
	...
	for(i = 0 ; i < sizeof(input)/sizeof(float) ; i++) {
		for(j = 0 ; j < sizeof(input)/sizeof(float) ; j++) {
			...
			//printf("float mul ua = %x, ub = %x, res = %x, res_fpu = %x\n", a.val, b.val, res.val, res_fpu.val);
			//printf("a       sign %x, exp %x, %d, sig %x\n", a.sign, a.exponent, a.exponent, a.significand);
			//printf("b       sign %x, exp %x, %d, sig %x\n", b.sign, b.exponent, b.exponent, b.significand);
			//printf("res     sign %x, exp %x\n", res.sign, res.exponent);
			//printf("res_fpu sign %x, exp %x\n", res_fpu.sign, res_fpu.exponent);
			assert(res_fpu.val == res.val);
		}
	}
